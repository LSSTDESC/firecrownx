    ...
    def do_cumulative(self, a, b):
        ...
        for x in self.x_data:
            if x >= a and x <= b:
                ...
                cumulative += (
                    result[2][1] ** 2 * result[1][0] / 2
                    + result[2][1] * result[1][1]
                    - result[2][0] ** 2 * result[1][0] / 2
        ...
        for knot in range(0, len(self.cumulative_data)):
            ...
            if cumulative_i >= c:
                ...
                return [
                    new_point,
                    (coef_a, coef_b),
                    (self.x_data[knot], self.x_data[knot + 1]),
                    c,
                ]
...
class PDF:
    ...
    def gaussian_oned_pdf(self, x, mean, sigma):
        return np.exp(-np.power(x - mean, 2.0) / (2 * np.power(sigma, 2.0))) * (
    ...
    def gaussian_twod(self, x, y, meanx, meany, sigmax, sigmay):
        gaus_x = np.exp(-np.power(x - meanx, 2.0) / (2 * np.power(sigmax, 2.0))) * (
            1 / (np.sqrt(2 * np.pi) * sigmax)
        )
        gaus_y = np.exp(-np.power(y - meany, 2.0) / (2 * np.power(sigmay, 2.0))) * (
            1 / (np.sqrt(2 * np.pi) * sigmay)
        )
